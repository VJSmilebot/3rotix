...
export async function getServerSideProps({ query }) {
  ...
  try {
    // Query Prisma User model instead of Supabase profiles table
    const users = await prisma.user.findMany({
      where: {
        handle: { not: null },
        role: 'CREATOR' // Only show creators
      },
      select: {
        id: true,
        handle: true,
    ...
    errorInfo = { message: 'Database fetch failed', detail: String(err.message) };
    console.error('[CREATORS] Database error:', err);
  }

  return {
    props: {
      initialProfiles: JSON.parse(JSON.stringify(rows)), // Serialize dates
