...
export async function getServerSideProps({ params }) {
  ...
  // Server-only import (prevents bundling weirdness)
  ...
  const profile = await prisma.user.findUnique({
    ...
    select: {
      id: true,
      supabaseId: true, // ✅ needed for ownership check
      name: true,
      handle: true,
      bio: true,
      image: true,
...
export default function PublicProfile({ profile }) {
  ...
  useEffect(() => {
    ...
    async function checkOwnership() {
      ...
      if (!dead && data?.user && profile?.supabaseId && data.user.id === profile.supabaseId) {
        setIsOwner(true);
      }
    }
    ...
    // ✅ key: depend on supabaseId, not prisma id
  }, [profile?.supabaseId, supabase]);
    ...
    async function fetchContent() {
      ...
      try {
        ...
        const [videosData, audioData, imagesData] = await Promise.all([
          videosRes.json().catch(() => null),
          audioRes.json().catch(() => null),
          imagesRes.json().catch(() => null),
        ]);

        // Normalize any API shape into an array so .map never crashes
        const normalize = (x) =>
          Array.isArray(x)
            ? x
        ...
        if (!dead) {
          setVideos(normalize(videosData));
          setAudio(normalize(audioData));
          setImages(normalize(imagesData));
        }
      } catch (err) {
  ...
  async function handleSendTip() {
    const amtNum = parseInt(tipAmount, 10);
    ...
    try {
      ...
      alert(`🎉 Tip sent! You tipped ${amtNum} Lipz to ${profile.name || profile.handle}`);
    ...
    <>
      ...
      <div style={{ maxWidth: 980, margin: "24px auto", padding: 20 }}>
        <div style={{ position: "relative" }}>
          {isOwner && (
            ...
            >
              <Link href="/creator" style={editProfileBtn} className="flex items-center gap-2">
        ...
        <div style={{ marginTop: 24 }}>
          {loading ? (
            <div style={{ textAlign: "center", padding: 40, color: "#666" }}>Loading content...</div>
          ) : (
            <>
              {activeTab === "videos" &&
                (videos.length === 0 ? (
                  <div style={{ textAlign: "center", padding: 40, color: "#666" }}>No public videos yet.</div>
                ) : (
                  <div
                    style={{
                      display: "grid",
                    ...
                    {videos.map((video) => (
                      ...
                      >
                        ...
                        <div style={{ padding: 12 }}>
                          ...
                          {video.description && (
                            <p style={{ margin: "4px 0 0", fontSize: 12, color: "#999" }}>{video.description}</p>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              {activeTab === "audio" &&
                (audio.length === 0 ? (
                  <div style={{ textAlign: "center", padding: 40, color: "#666" }}>No public audio yet.</div>
                ) : (
                  <div
                    style={{
                      display: "grid",
              ...
              {activeTab === "images" && (
                (() => {
                  ...
                  return list.length === 0 ? (
                    <div style={{ textAlign: "center", padding: 40, color: "#666" }}>No public images yet.</div>
                  ) : (
                    <div
                      style={{
                        display: "grid",
                      ...
                      {list.map((img) => (
                        ...
                        >
                          ...
                          <div style={{ padding: 12 }}>
                            ...
                            {img.description ? (
                              <p style={{ margin: "4px 0 0", fontSize: 12, color: "#999" }}>{img.description}</p>
                            ) : null}
                          </div>
                        </div>
                      ))}
              ...
              {activeTab === "streams" && (
                <div style={{ textAlign: "center", padding: 40, color: "#666" }}>Past streams coming soon.</div>
              )}
            </>
          )}
        </div>
      ...
      {showTipModal && (
        ...
        >
          ...
          >
            <h2 style={{ margin: "0 0 16px", fontSize: 20 }}>Send Tip to {name || handle}</h2>

            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", marginBottom: 8, fontSize: 14, color: "#999" }}>Lipz Amount</label>
              <input
                type="number"
                min="1"
                value={tipAmount}
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", marginBottom: 8, fontSize: 14, color: "#999" }}>Message (optional)</label>
              <textarea
                value={tipMessage}
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  color: "#fff",
                  cursor: tipping || !tipAmount || parseInt(tipAmount, 10) <= 0 ? "not-allowed" : "pointer",
                  fontSize: 14,
                  fontWeight: "bold",
                  opacity: tipping || !tipAmount || parseInt(tipAmount, 10) <= 0 ? 0.5 : 1,
                }}
              >
                {tipping ? "Sending..." : `Send ${parseInt(tipAmount, 10)} Lipz`}
              </button>
            </div>
          </div>
        </div>
